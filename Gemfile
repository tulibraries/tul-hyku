...
# If you need to override a gem that is specified in Hyku, you need to
# completely replace the Gemfile. This is an anti-pattern and should be avoided
# prefer to improve Hyku with your changes if at all possible
if ENV.fetch('HYKU_KNAPSACK_GEMFILE_REPLACEMENT_PATH', false)
  eval_gemfile ENV.fetch('HYKU_KNAPSACK_GEMFILE_ADDITIONS_PATH')
else
  ...
  # If you are only adding gems or changing branches of o Git based gem, then you can use additions instead of replacing the
  # whole Gemfile. This is the preferred method of adding gems to your Hyku instance.
  eval_gemfile ENV.fetch('HYKU_KNAPSACK_GEMFILE_ADDITIONS_PATH') if ENV.fetch('HYKU_KNAPSACK_GEMFILE_ADDITIONS_PATH', false)
end
